 ...
 * The parent is a mandatory constructor level reference, so this registry only exists as long as
 * the parent it was built on: a parent that goes away or a changed target filter takes this
 * registry service down with it, instead of silently changing the set of resolvable packages
 * underneath everyone who is currently using it.
 ...
 */
@Component(service = {}, configurationPid=EMFNamespaces.EPACKAGE_REGISTRY_CONFIG_NAME, configurationPolicy=ConfigurationPolicy.REQUIRE)
@Designate(ocd = EPackageRegistryConfig.class, factory = true)
@ProviderType
@ServiceCapability(EPackage.Registry.class)
public class ConfigurationEPackageRegistryComponent extends SelfRegisteringServiceComponent implements RegistryPropertyListener {
